...
from PolicyGradient_RL1 import PolicyGradient
print("Brain: PolicyGradient_RL1")
print("using FPO")
import argparse
...
def train(params):
    ...
    while epoch_i < params['epochs']:
        ...
        if (epoch_i % batch_size == 0) & (epoch_i > 1):
            ...
            print("End2End time: {}".format(time_ep_al))
        ...
        epoch_i += 1
        time_ep_al += time.time() - time_ep_s
        if epoch_i > 6000:
            # batch_size = 100
            batch_size = 40
...
def main():
    ...
    params['path'] += str(args.batch_choice)
