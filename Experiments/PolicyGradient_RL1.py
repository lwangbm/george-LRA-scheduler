...
class PolicyGradient:
    ...
    def learn(self, epoch_i, entropy_weight, IfPrint=False, alpha=0.5):
        ...
        if np.isnan(loss).any():
            test=1
        if IfPrint:
            print("epoch: %d, safety_batch: %f, ep_ss: %f, tput: %f, entropy: %f, loss: %f" % (
                epoch_i, np.mean(self.ss_batch), np.mean(self.ep_ss), np.mean(self.tput_batch), np.mean(entropy), np.mean(loss)))
